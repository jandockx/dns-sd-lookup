 ...
 * - we remove a starting '_', if it is there
 */
  ...
  post: [
    function (fqdn, result) {
      const matched = fqdn.match(
        new RegExp(
          `^(?:_?)(.*)\\._sub\\._${regexEscape(this.type(fqdn))}\\._${this.protocol(fqdn)}\\.${regexEscape(this.domain(fqdn))}$`
        )
      )
...
.implementation(function extractDomain (fqdn) {
  ...
  let subtype = parts.slice(0, Math.max(lastTcp, lastUdp, 2) - 2).join('.')
  subtype = /^_?(.*)$/.exec(subtype)[1]
